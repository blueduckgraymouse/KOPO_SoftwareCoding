...
/* 요구사항
	...
	-> 리펙토링
		- 배열의 크기를 const변수로 변경 - 각 로직에서 반복문을 그 const변수에 따라 제어되도록
		ex) 티텟 정가의 종류가 청소년1, 청소년2로 나뉘고 금액도 변동될 때 로직은 안 건드릭 전역변수로 선언된 const상수와 각 배열 내 값만 바꾸면 되도록.	 
		- 결과 예시의 report.csv처럼 구분번호가 아닌 구분번호에 따른 한글 매핑을 몰라도 되도록 데이터를 한글로 저장
*/
