 ...
 *
 */
public class K35_ex01_p6 {
	// 영수증에 출력할 상품 정보
	static String[] k35_itemname = {"해피홈 SAFE365 핸드워시(펌프형)", "(G)LH원형유리화병 100ml", "건포도", "오렌지주스", "초코에몽", "abc초콜렛", "서울우유" ,"크래미", "포카칩", "프링글스 오리지날",
									...
									"공기청정기", "스피커"};
	static int[] k35_price = 		{10000, 1200, 13000, 3000, 1000, 500, 3800, 3300, 2000, 1700,
									10000, 1500, 9900, 7900, 11900, 4200, 5100, 17000, 213200, 12000000,
									79300, 139000, 32190, 27890, 65900, 54190, 9900, 2500, 19280, 3510,
									342010, 98000};
	static int[] k35_num = 			{2, 8, 1, 2, 3, 4, 1, 1, 2, 1,
									1, 2, 1, 1, 2, 2, 1, 1, 1, 1,
									1, 1, 2, 2, 1, 1, 3, 10, 3, 23,
									1, 59};
	static boolean[] k35_taxfree = {false, false, true, false, false, false, false, false, false, false,
									true, true, true, true, false, false, false, false, false, false,
									false, false, true, true, false, false, false, true, true, false,
									false, false};
	
	// 금액, 날짜 관련 객체 선언 및 초기화
	static DecimalFormat k35_df = new DecimalFormat("###,###,###,###,###");
	static Calendar k35_calt = Calendar.getInstance();							// 기본 타임존과 지역에 대해 날짜정보(현재 시간)를 가져와 calt라는 변수에 저장
	static SimpleDateFormat k35_sdt = new SimpleDateFormat("YYYY-MM-dd HH:mm");	// 연도 4자리, 월 2자리, 일 2자리, 시간 2자리, 분 2자리의 시간 형식을 sdt객라는 객체로 생성
	static SimpleDateFormat k35_sdt2 = new SimpleDateFormat("YYYYMMdd");		// 연도 4자리, 월 2자리, 일 2자리의 날짜 형식을 sdt객라는 객체로 생성
	
	// 영수증 계산에 필요한 변수 선언 및 초기화
	static int k35_sum_price_total = 0;											// 모든 물품의 총 합계
	static int k35_sum_price_tax = 0;											// 세금이 부과되는 제품의 합계
	static int k35_sum_price_taxFree = 0;										// 세금이 부과되지 않는 제품의 합계
	static double k35_tax_rate = 0.1;											// 세율 10%로 설정
	static int k35_sum_price_before_tax = 0;									// 세금이 부과되는 제품의 합계의 세전 합계이 저장될 변수
	static int k35_tax = 0;														// 세금이 부과되는 제품의 합계의 세금이 저장될 변수

	public static void main(String[] args) {
		int k35_itemCount = k35_itemname.length;
		k35_titlePrint();											// 영수증 상단 정보 출력 함수 호출 (매장 정보 등등)
		k35_timestemp();											// 날짜 출력 함수 호출
		k35_headerPrint();											// 상품 출력단의 헤더 출력 함수 호출
		for (int k35_i = 0 ; k35_i < k35_itemCount ; k35_i++)		// 모든 상품 출력
			k35_itemPrint(k35_i);									// 상품 정보 출력 함수 호출
		k35_totalPrint();											// 영수증 하단 정보 출력 함소 호출 (총 가격, 세금, 기타 등등)
	}
	
	/**
	 *  영수증 하단에 총액, 부가세, 포인트 등의 정보르 출력한다.
	 */
	private static void k35_totalPrint() {
	...
	/**
	 *  하나의 상품에 대해 한 줄의 상품 구매 정보(상품명, 수량, 가격 등)을 출력한다.
	 * @param k35_i	: 현재 출력해야 하는 상품 정보가 담긴 배열들의 index번호
	 */
	private static void k35_itemPrint(int k35_i) {
	...
	/**
	 *  구매 상품을 나열하기 전에 위에 헤더 정보를 출력한다.
	 */
	private static void k35_headerPrint() {
		System.out.printf("-----------------------------------------\n");
	...
	/**
	 *  영수증의 현재시간이 출력되야하는 줄을 출력한다.
	 */
	private static void k35_timestemp() {
		System.out.printf("%s%-22s%12s\n", "[구 매]", k35_sdt.format(k35_calt.getTime()), "POS:001-9861");	// 41 = 5+2 + (6+)16 + 12
	}
	
	/**
	 *  영수증의 상단의 매장명, 매장 정보 등을 출력한다.
	 */
	private static void k35_titlePrint() {
