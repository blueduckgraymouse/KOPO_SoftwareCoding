 ...
 *
 */
public class K35_ex11_re {
	public static void main(String[] args) {
		// 변수 선언
		int k35_iNumVal = 1000000000;												// 한글로 읽을 숫자를 int형 변수 iNumVal에 초기화
		...
		while(true) {																// 무한 반복
			...
			if (k35_digit.equals("0")) {											// 지금 loop에서 읽어야 하는 자리의 숫자 한 개(digit)가 0이면
				if ((k35_unitX[k35_j].equals("만") || k35_unitX[k35_j].equals("억")) //   '만'의 자리나 '억'의 자리 일 경우 0이어도 이전에 출력된 일십(만)~구백(만) 혹은 일십(억)~구백(억)을 위해서 읽어줘야 한다.
						&& !k35_sNumVoice.substring(k35_sNumVoice.length()-1).equals("억")) { // 만~백만 단위에 해당하는 숫자가 비였을 때 '만'이 출력되는 것을 방지하기 위해, '만'이 찍히려고 할 때 sNumVoice의 마지막 글자가 억이면 안 찍히도록 설정
