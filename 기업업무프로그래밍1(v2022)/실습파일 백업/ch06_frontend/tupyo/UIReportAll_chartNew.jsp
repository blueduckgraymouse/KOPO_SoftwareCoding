...
<body>
	...
	<script>
		...
		// 차트 데이터 클릭시 해당 라벨인 후보의 연령대별 득표수/득표율 확인하는 UIReportOne_charNew.jsp로 이동
		document.getElementById("ReportAll").onclick = function(evt) {
			var activePoint = myChart.getElementsAtEventForMode(evt, 'nearest', { intersect: true }, true); // getElementsAtEventForMode(e, mode, options, useFinalPosition)
			//var activePoint = myChart.getElementsAtEvent(evt); 	// is not a function 에러
			if (activePoint.length > 0) {
				var clickedElementindex = activePoint[0].index;
				var label = myChart.data.labels[clickedElementindex];  
				var id = label.substr(0,1);
				window.location = "http://192.168.23.94:8003/ch06/tupyo/UIReportOne_chartNew.jsp?id_hubo=" + id;
			}
		};
	</script>
</body>
