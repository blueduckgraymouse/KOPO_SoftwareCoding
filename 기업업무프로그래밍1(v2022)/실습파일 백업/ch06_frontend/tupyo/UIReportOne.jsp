		...
		<table border=1 id="bar">
			<%
				for (int i = 0 ; i < tag_ageRange.getLength() ; i++) {
					...
					out.println("<tr><td width=80>" + ageRange + "0대</td>");
					if(count == 0) {
						out.println("<td width=450 height=15>" + count + "표 (" + form.format(rate*100) + "%)</td>");	// 득표수가 0일 경우 득표수만 출력
					} else {
						out.println("<td width=450 height=15><hr color='red' width=" + (int)(rate*300) + "height=15/>&nbsp;&nbsp;" + count + "표 (" + form.format(rate*100) + "%)</td>");
					}																						// 득표수가 0일 경우 득표수와 바 출력
				}
			%>
		</table>
	</div>
