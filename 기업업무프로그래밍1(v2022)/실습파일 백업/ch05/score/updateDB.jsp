...
<head>
	...
	<%
		...
		String name_str = request.getParameter("name_str");		// 이전 페이지의 모든 input태그의 데이터를 파라미터로 저장
		...
		String query1 = "update examtable set name='" + name_str +	// 전달 받은 파라미터의 값으로 수정 쿼리문 완성하여 변수query1에 저장
						"', kor=" + scores[0] + 
						", eng=" + scores[1] + 
						", mat=" + scores[2] + 
						" where studentid= " + id_str + ";";
		stmt.execute(query1);								// 쿼리문 실행
		...
		String query2 = "select *, " +
						...
						"from examtable as t;";				// examtable 테이블 조회 쿼리 query2변수에 저장
	...
	<div align=center>
		...
		<%
			while (rset.next()) {						// ResultSet에 저장된 모든 레코드에 대하여 반복
				if (rset.getInt(2) == Integer.parseInt(id_str)) {		// 출력할 레코드가 방금 수정한 레코드의 id이면 노란 배경 처리
					out.println("<tr height=50px bgcolor=yellow>");
				} else {
