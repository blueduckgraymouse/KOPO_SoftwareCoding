...
select 
	case 										# case문을 통하여 age에 따라 00대~ 90대까지 구분 구룹화한 후 연령대와 득표수, 득표율 조회 하되,
		when age between 0 and 10 then '00대'									# 이름이 정연인 경우만 득표수 기준의 내림차순으로 조회
	...
	count(*) as `특표수`,	
    (count(name) / (select count(*) from tupyo) * 100) as '특표율'
    from tupyo where name = '정연' group by `연령대` order by 특표수 desc;	
...
select 	name as 이름,
	case 
        ...
        count(1) as `특표수`,
		(count(name) / (select count(*) from tupyo) * 100) as '특표율'
        from tupyo group by `연령대`, name order by 이름, 연령대;
...
drop table if exists report_examtable;
create table report_examtable as						# report_examtable 테이블 생성
	select name, id, kor, eng, mat,						# 모든 필드,
	(t.kor+t.eng+t.mat) as sum,							# 합계 점수,
    (t.kor+t.eng+t.mat)/3 as ave,						# 평균 점수,
    (select count(*)+1 from examtable as r where r.kor+r.eng+r.mat > sum) as rank_sum	# 현재 래코드의 총점보다 높은 레코드의 개수를 조회하여 +1 한 값을 rank_sum이라 별칭 하여 조회
	from examtable as t;
select * from report_examtable;							# report_examtable 테이블 조회
...
#3
select * from report_examtable order by rank_sum;	#report_examtable테이블을 둥수기준 오름차순으로 정렬
...
# 3개의 테이블(목록, 현재 합계, 누적합계)를 생성하는 프로시저
drop procedure if exists print_report;				# print_report 프로시저가 존재하면 삭제
delimiter $$
create procedure print_report(no_page integer, size_page integer)	# integer타입의 no_page 변수와 size_page 변수를 파라미터로 받는 print_report 프로시저 생성
begin												# 프로시저 내용의 시작을 명시
	declare size_tt integer;						# 성적 테이블의 총 레코드 수가 저장될 변수 선언
    ...
    declare no_end integer;							# 현재 페이지의 마지막 레코드 인덱스 번호가 저장될 변수 선언 , limit을 통해서 현재 페이지까지의 누적 정보를 조회하기 위해 필요
    
    # 페이지 처리를 위한 전처리
    select count(*) into size_tt from examtable;	# examtable 테이블의 총 레코드 수를 조회하여 변수 size_tt에 저장
    ...
    set no_end = no_start + size_page;				# 누적 페이지 정보 조회의 limit에서 사용될 마지막 인덱스 번호 계산 <= (limit 0, no_end)

	# 현재 페이지 데이터 목록 테이블
	drop table if exists list_score;	# list_score 테이블 존재하면 삭제
	create table list_score as			# 성적 목록이 저장될 list_score 테이블 생성
		select *, (kor+eng+mat) as sum, (kor+eng+mat)/3 as ave from examtable limit no_start , size_page;	# 현재 페이지에 해당하는 데이터 목록만 list_score에 저장
        
	# 현재 페이지 현황 테이블
	drop table if exists info_page;		# info_page 테이블 존재하면 삭제
	CREATE TABLE info_page as			# 현재 페이지 성적 현황이 저장될 info_page 테이블 생성
        select sum(kor) as kor_sum, sum(eng) as eng_sum, sum(mat) as mat_sum, sum(kor+eng+mat) as sum_sum, sum((kor+eng+mat)/3) as sum_avg,	# 현재 페이지의 합계 정보와
			avg(kor) as avg_kor, avg(eng) as avg_eng, avg(mat) as avg_mat, avg(kor+eng+mat) as avg_sum, avg((kor+eng+mat)/3) as avg_avg 	# 현재 페이지의 평균 정보를
            from (select * from score limit no_start , size_page) as apage;		# 현재 페이지에 해당하는 데이터에 한해 조회
            
	# 누적 페이지 현황 테이블
	drop table if exists info_page_total;	# info_page_total 테이블 존재하면 삭제 
	CREATE TABLE info_page_total as			# 누적 페이지 성적 현황이 저장될 info_page_total 테이블 생성
        select sum(kor) as kor_sum, sum(eng) as eng_sum, sum(mat) as mat_sum, sum(kor+eng+mat) as sum_sum, sum((kor+eng+mat)/3) as sum_avg,	# 누적 페이지의 합계 정보와
			avg(kor) as avg_kor, avg(eng) as avg_eng, avg(mat) as avg_mat, avg(kor+eng+mat) as avg_sum, avg((kor+eng+mat)/3) as avg_avg  	# 누적 페이지의 평균 정보를
            from (select * from score limit 0 , no_end) as apage;				# 처음부터 현재 페이지 마지막 번호까지의 데이터에 한해 조회
end $$										# delimiter가  끝남을 명시
delimiter ;

call print_report(5, 25);		# 25개 데이터를 기준으로 했을 때 5페이지에 해당하는 성적집계표 출력(테이블 생성)하는 프로시저 호출
select * from list_score;		# 성적 목록 확인
select * from info_page;		# 현재 페이지 합계와 평균 정보 확인
select * from info_page_total;	# 누적 페이지의 합계와 평균 정보 확인

call print_report(-1, 25);		# 현재 페이지 번호가 1보다 작을 때
select * from list_score;		# 성적 목록 확인

call print_report(2000, 25);	# 현재 페이지 번호가 마지막 페이지 번호보다 클 때
select * from list_score;		# 성적 목록 확인

#2
# 3개의 테이블을 생성하는 프로시저 - 등수 추가
drop procedure if exists print_report_with_rank;	# print_report_with_rank 프로시저가 존재하면 삭제
delimiter $$
create procedure print_report_with_rank(no_page integer, size_page integer)	# integer타입의 no_page 변수와 size_page 변수를 파라미터로 받는 print_report_with_rank 프로시저 생성
begin												# 프로시저 내용의 시작을 명시
	declare size_tt integer;						# 성적 테이블의 총 레코드 수가 저장될 변수 선언
    declare no_max_pg integer;						# 마지막 페이지 번호가 저장될 변수 선언 , 파라미터에 마지막 성적 페이지 번호보다 큰 값이 들어왔는지 확인 하기 위해 필요.
    declare no_start integer;						# limit에서 조회 시작할 인덱스 번호를 나타내는 변수 선언
    ...
    # 페이지 처리를 위한 전처리
    select count(*) into size_tt from examtable;	# examtable 테이블의 총 레코드 개수 조회
    set no_max_pg = size_tt / size_page;			# 마지막 페이지 번호 계산
    if no_page < 1 then								# 파라미터로 들어온 페이지 번호가 1보다 작으면
		set no_page = 1;							# 	1 페이지로 조회하기 위해 no_page 변수에 0로 저장
	elseif no_page>no_max_pg then					# 파라미터로 들어온 페이지 번호가 마지막 페이지 번호보다 크면
	...
	# 현재 페이지 데이터 목록 테이블
	drop table if exists list_score_rank;	# list_score 테이블 존재하면 삭제
	create table list_score_rank as			# 성적 목록이 저장될 list_score 테이블 생성
		select *, (kor+eng+mat) as sum, (kor+eng+mat)/3 as ave,									# 기존 모든 필드에 합계, 평균 필드 추가,
			(select count(*)+1 from examtable as r where r.kor+r.eng+r.mat > sum) as rank_sum	# 현재 래코드의 총점보다 높은 레코드의 개수를 조회하여 +1 한 값을 rank_sum이라 별칭하는 필드 추가
			from examtable limit no_start, size_page;											# 현재 페이지에 해당하는 데이터 목록만 list_score에 저장
    
	# 현재 페이지 현황 테이블
	drop table if exists info_page_rank;		# info_page 테이블 존재하면 삭제
	CREATE TABLE info_page_rank as			# 현재 페이지 성적 현황이 저장될 info_page 테이블 생성
        select sum(kor) as kor_sum, sum(eng) as eng_sum, sum(mat) as mat_sum, sum(kor+eng+mat) as sum_sum, sum((kor+eng+mat)/3) as sum_avg,	# 현재 페이지의 합계 정보와
			avg(kor) as avg_kor, avg(eng) as avg_eng, avg(mat) as avg_mat, avg(kor+eng+mat) as avg_sum, avg((kor+eng+mat)/3) as avg_avg 	# 현재 페이지의 평균 정보를
            from (select * from score limit no_start , size_page) as apage;		# 현재 페이지에 해당하는 데이터에 한해 조회
            
	# 누적 페이지 현황 테이블
	drop table if exists info_page_total_rank;	# info_page_total 테이블 존재하면 삭제 
	CREATE TABLE info_page_total_rank as			# 누적 페이지 성적 현황이 저장될 info_page_total 테이블 생성
        select sum(kor) as kor_sum, sum(eng) as eng_sum, sum(mat) as mat_sum, sum(kor+eng+mat) as sum_sum, sum((kor+eng+mat)/3) as sum_avg,	# 누적 페이지의 합계 정보와
			avg(kor) as avg_kor, avg(eng) as avg_eng, avg(mat) as avg_mat, avg(kor+eng+mat) as avg_sum, avg((kor+eng+mat)/3) as avg_avg  	# 누적 페이지의 평균 정보를
            from (select * from score limit 0 , no_end) as apage;				# 처음부터 현재 페이지 마지막 번호까지의 데이터에 한해 조회
end $$										# delimiter가  끝남을 명시
...
call print_report_with_rank(5, 25);	 # 25개 데이터를 기준으로 했을 때 5페이지에 해당하는 성적집계표 출력(테이블 생성)하는 프로시저 호출

select * from list_score_with_rank;	# 성적 목록 확인
select * from info_page_rank;		# 현재 페이지 합계와 평균 정보 확인
select * from info_page_total_rank;	# 누적 페이지의 합계와 평균 정보 확인


-- 위에꺼 원본, 주석없는 버전
-- drop procedure if exists print_report_with_rank;
-- delimiter $$
...
begin								# 프로시저 내용의 시작을 알림
	...
	declare lat_now double default 37.2877;	# 집의 위도 변수로 초기화
	declare lon_now double default 127.0569; # 집의 경도 변수로 초기화
...
begin									# 프로지서 내용의 시작을 알림
	...
	create table list_freewifi as       			# 현재 페이지의 무료 와이파이 목록만 저장될 list_freewifi 테이블 생성
		select inst_place, inst_place_detail, inst_city, 	 # 무료 와이파이존의 필수 몇 가지 필드 추가
        inst_country, place_addr_land, latitude, longitude,
        f_dis(latitude, longitude) as distance_km	# 위도와 경도로 f_dis 함수 호출하여 계산된 거리값 필드 추가
        from freewifi limit no_start , size_page;	# 현재 페이지에 한하여 조회
end $$					# 프로시저와 delimiter의 끝을 명시

call print_freewifi(5,25);				# 25개의 레코드를 기준으로 헀을 때 5페이지에 해당하는 데이터를 테이블로 생성하는 프로시저 호출
select * from list_freewifi;			# 생성된 테이블 확인
select * from freewifi limit 100, 25;	# 제대로 된 데이터로 생성한 것인지 확인
